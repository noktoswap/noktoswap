...
/// Deploys NoktoSwap to an identical address on every chain, via CreateX's CREATE3.
///
...
///
///   forge script script/NoktoSwapCreate3.s.sol:NoktoSwapCreate3 --sig 'predict()' --rpc-url sepolia
///   forge script script/NoktoSwapCreate3.s.sol:NoktoSwapCreate3 --rpc-url sepolia --broadcast
///
/// `predict()` needs no key — set `NOKTOSWAP_DEPLOYER` to the address alone.
contract NoktoSwapCreate3 is Script {
    ICreateX constant CREATEX = ICreateX(0xba5Ed099633D3B313e4D5F7bdc1305d3c28ba5Ed);

    /// Low 11 bytes of the salt — ascii "noktoswapv1".
    uint88 constant ENTROPY = 0x6e6f6b746f737761707631;

    function parameters() internal pure returns (NoktoSwap.Parameters memory) {
    ...
    function _initCode(address owner) internal pure returns (bytes memory) {
        return abi.encodePacked(type(NoktoSwap).creationCode, abi.encode(parameters(), owner));
    }

    function _deployer() internal view returns (address) {
        address given = vm.envOr("NOKTOSWAP_DEPLOYER", address(0));
        if (given != address(0)) return given;
        return vm.addr(vm.envUint("DEPLOYER_KEY"));
    }

    /// Defaults to the deployer, deliberately. NoktoSwapDeployer defaults instead
    ...
    function _owner(address deployer) internal view returns (address) {
        return vm.envOr("NOKTOSWAP_OWNER", deployer);
    }
